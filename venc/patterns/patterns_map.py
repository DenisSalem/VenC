...
class PatternsMap:
    ...
    NON_CONTEXTUALS = {  # all of the below is loaded from datastore
        ...
        "extra": {  # Loaded from function localy imported
            "Audio": "venc.patterns.theme.get_audio",
            "CodeHighlight": "venc.patterns.third_party_wrapped_features.pygmentize.highlight",
            "CodeHighlightInclude": "venc.patterns.third_party_wrapped_features.pygmentize.highlight_include",
            "DisableMarkup": "venc.patterns.non_contextual.disable_markup",
            "Escape": "venc.patterns.non_contextual.escape",
            "GetEmbedContent": "venc.patterns.third_party_wrapped_features.oembed.wrapper_embed_content",
            "GetVenCVersion": "venc.patterns.non_contextual.get_venc_version",
            "HTML": "venc.patterns.non_contextual.html",
            "IncludeFile": "venc.patterns.non_contextual.include_file",
            "IncludeFileIfExists": "venc.patterns.non_contextual.include_file_if_exists",
            "Kroki": "venc.patterns.third_party_wrapped_features.kroki.kroki",
            "KrokiFromFile": "venc.patterns.third_party_wrapped_features.kroki.kroki_from_file",
            "Latex2MathML": "venc.patterns.third_party_wrapped_features.latex2mathml.latex_2_mathml",
            "SetColor": "venc.patterns.non_contextual.set_color",
            "SetStyle": "venc.patterns.non_contextual.set_style",
            "Table": "venc.patterns.non_contextual.table",
            "Video": "venc.patterns.theme.get_video",
        },
    }
    ...
    def __init__(self):
        ...
        for pattern_name in PatternsMap.NON_CONTEXTUALS["extra"].keys():
            ...
            self.non_contextual["extra"][pattern_name] = getattr(
                importlib.import_module(module, package), function
            )

        from venc.datastore import datastore

        for pattern_name in PatternsMap.NON_CONTEXTUALS["blog"].keys():
            self.non_contextual["blog"][pattern_name] = getattr(
                datastore, PatternsMap.NON_CONTEXTUALS["blog"][pattern_name]
            )

        for pattern_name in PatternsMap.NON_CONTEXTUALS["entries"].keys():
            self.non_contextual["entries"][pattern_name] = getattr(
                datastore, PatternsMap.NON_CONTEXTUALS["entries"][pattern_name]
            )

        for pattern_name in PatternsMap.NON_PARALLELIZABLES.keys():
            self.non_contextual["non_parallelizable"][pattern_name] = getattr(
                datastore, PatternsMap.NON_PARALLELIZABLES[pattern_name]
            )
