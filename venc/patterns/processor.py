...
from venc.patterns.patterns_map import PatternsMap
...
class Pattern:
    ...
    def __init__(self, s, o, c, sub_patterns, root):
        ...
        if len(sub_patterns) and sub_patterns[0].o > o and sub_patterns[0].c < offset:
            ...
            raise VenCException(
                (
                    "this_pattern_is_embed_in_the_name_of_another_one",
                    sub_patterns[0].payload[0],
                ),
                sub_patterns[0],
            )
        ...
        for key in PatternsMap.NON_CONTEXTUALS.keys():
            if (
                key != "entries"
                and pattern_name in PatternsMap.NON_CONTEXTUALS[key].keys()
            ):
...
class PatternTree:
    def __init__(self, string, context="", has_markup_language=False):
        ...
        self.sub_patterns = self.__build_tree(self.__get_boundaries(string))
        ...
        for pattern in self.unknown_patterns:
            if (
                type(pattern.parent) == PatternTree
                or pattern.parent.payload[0] != "Escape"
            ):
                from venc.exceptions import UnknownPattern
    ...
    def __get_boundaries(self, string):
        o = [
            o
            for o in PatternTree.__find_pattern_boundaries(
                string, ".:", Boundary.BONDARY_TYPE_OPENING
            )
        ]
        c = [
            c
            for c in PatternTree.__find_pattern_boundaries(
                string, ":.", Boundary.BONDARY_TYPE_CLOSING
            )
        ]

        if len(o) != len(c):
            from venc.exceptions import MalformedPatterns

            raise MalformedPatterns(self, o, c)

        return tuple(sorted(o + c, key=lambda x: x.index))

    def __get_boundaries_block(self, boundaries, start, offset):
        if boundaries[start].boundary_type == Boundary.BONDARY_TYPE_CLOSING:
            from venc.exceptions import VenCSyntaxError

            raise VenCSyntaxError(
                self,
                boundaries[start].index + offset,
                boundaries[start].index + 2 + offset,
            )
    ...
    def __build_tree(
        self, boundaries, start=0, limit=None, previous_end=None, offset=0
    ):
        if not limit:
        ...
        while start < limit:
            ...
            if end - start - 1 > 0:
                offset, pattern = self.__build_tree(
                    boundaries, start + 1, end - 1, end, offset
                )
                ...
                offset += self.__apply_and_compute_offset_and_check_parallelizable(
                    pattern
                )
                sub_patterns_append(pattern)
    ...
    def match_pattern_flags(self, flag, sub_patterns=None):
        nodes = self.sub_patterns if sub_patterns is None else sub_patterns
        pattern_names = []
        for pattern in nodes:
            if flag & pattern.flags:
                pattern_names.append(pattern)
            pattern_names += self.match_pattern_flags(
                flag, sub_patterns=pattern.sub_patterns
            )
...
class Processor:
    ...
    def apply_pattern(
        self, parent, pattern, flags, payload_offset, recursion_error_triggered_by
    ):
        if recursion_error_triggered_by == pattern.name_id:
            from venc.exceptions import VenCException

            raise VenCException(
                ("pattern_recursion_error", pattern.payload[0]), pattern
            )
        ...
        if (pattern.flags & (flags ^ Pattern.FLAG_NON_PARALLELIZABLE)) and (
            (flags & Pattern.FLAG_NON_PARALLELIZABLE)
            or (not (pattern.flags & Pattern.FLAG_NON_PARALLELIZABLE))
        ):
            try:
                chunk = self.functions[pattern_name](pattern, *args)

            except TypeError as e:
                from venc.exceptions import WrongPatternArgumentsNumber

                raise WrongPatternArgumentsNumber(
                    pattern, pattern.root, self.functions[pattern_name], args
                )
            ...
            if type(parent) == Pattern:
                ...
                parent.payload[pattern.payload_index] = (
                    ss[: pattern.o + payload_offset[0]]
                    + chunk
                    + ss[pattern.c + payload_offset[0] :]
                )
    ...
    def process(self, parent, flags, recursion_error_triggered_by=None):
        payload_offset = [0, 1]  # offset  # payload_index
        parent_sub_patterns_filtered = []
        parent_sub_patterns, parent_sub_patterns_filtered_append = (
            parent.sub_patterns,
            parent_sub_patterns_filtered.append,
        )
        i = 0
        while i < len(parent_sub_patterns):
            pattern = parent_sub_patterns[i]
            applied, leftovers = self.apply_pattern(
                parent, pattern, flags, payload_offset, recursion_error_triggered_by
            )
            if not applied:
