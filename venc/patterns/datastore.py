...
class DatastorePatterns:
    def if_categories(self, pattern, if_true, if_false=""):
        if (
            ...
            and not self.blog_configuration["disable_categories"]
        ):
            return if_true
    ...
    def if_chapters(self, pattern, if_true, if_false=""):
        if (
            self.chapters_index != []
            and not self.blog_configuration["disable_chapters"]
        ):
            return if_true
    ...
    def if_feeds_enabled(self, pattern, if_true, if_false=""):
        if (
            self.blog_configuration["disable_atom_feed"]
            and self.blog_configuration["disable_rss_feed"]
        ):
            return if_false
    ...
    def if_blog_metadata_is_true(self, pattern, metadata_name, if_true, if_false=""):
        return self.if_metadata_is_true(
            metadata_name, if_true, if_false, self.blog_configuration
        )

    def if_entry_metadata_is_true(self, pattern, metadata_name, if_true, if_false=""):
        return self.if_metadata_is_true(
            metadata_name, if_true, if_false, self.requested_entry
        )
    ...
    def get_chapters(self, pattern, list_open, item_open, item_close, list_close):
        ...
        if not key in self.html_chapters.keys():
            self.html_chapters[key] = self.build_html_chapters(
                list_open, item_open, item_close, list_close, self.chapters_index, 0
            )
    ...
    def get_entry_toc(self, pattern, open_ul, open_li, content, close_li, close_ul):
        ...
        if hasattr(self.requested_entry, "toc"):
            output = self.build_entry_html_toc(
                self.requested_entry.toc, open_ul, open_li, content, close_li, close_ul
            )
            return (
                "</p>" + output + "<p>"
    ...
    def get_blog_metadata(self, pattern, metadata_name):
        ...
        try:
            ...
            raise VenCException(
                ("blog_has_no_metadata_like", metadata_name), context=self
            )

    def get_blog_metadata_if_exists(
        self, pattern, metadata_name, if_true="", if_false="", ok_if_null=True
    ):
        try:
    ...
    def get_blog_metadata_if_not_null(
        self, pattern, metadata_name, if_true="", if_false=""
    ):
        try:
            return self.get_blog_metadata_if_exists(
                pattern, metadata_name, if_true, if_false, ok_if_null=False
            )
    ...
    def get_entry_attribute_by_id(self, pattern, attribute, identifier):
        ...
        if not key in self.cache_get_entry_attribute_by_id.keys():
            try:
                entry = [
                    entry for entry in self.entries if entry.id == int(identifier)
                ][0]
                ...
                raise VenCException(
                    ("cannot_retrieve_entry_attribute_because_wrong_id",), pattern
                )
    ...
    def get_entry_chapter_path(self, pattern):
        if self.blog_configuration["disable_chapters"]:
            ...
            from venc.datastore.metadata import Chapter

            return (
                self.requested_entry.chapter.path
                if hasattr(self.requested_entry, "chapter")
                and type(self.requested_entry.chapter) == Chapter
                else ""
            )
    ...
    def get_entry_metadata_if_exists(
        self, pattern, metadata_name, string="", string2="", ok_if_null=True
    ):
        """value"""
        try:
    ...
    def get_entry_metadata_if_not_null(
        self, pattern, metadata_name, string="", string2=""
    ):
        """value"""
        return self.get_entry_metadata_if_exists(
            pattern, metadata_name, string, string2, ok_if_null=False
        )
    ...
    def get_chapter_attribute_by_index(self, pattern, attribute, index):
        ...
        if not key in self.cache_get_chapter_attribute_by_index.keys():
            try:
                self.cache_get_chapter_attribute_by_index[key] = str(
                    getattr(self.raw_chapters[index].chapter, attribute)
                )
                ...
                raise VenCException(
                    ("chapter_has_no_attribute_like", attribute), pattern
                )
    ...
    def get_root_page(self, pattern):
        if self.root_page == None:
            self.root_page = "\x1a" + self.blog_configuration["path"][
                "index_file_name"
            ].format(**{"page_number": ""})

        return self.root_page

    def get_entry_categories_tree(
        self, pattern, open_node, open_branch, close_branch, clode_node
    ):
    ...
    def range_entries_by_id(self, pattern, begin_at, end_at):
        ...
        if not str(id(key)) in self.cache_entries_subset.keys():
            ...
            if end_at > begin_at:
                entries = [
                    entry for entry in self.entries if end_at >= entry.id >= begin_at
                ]
    ...
    def for_entries_set(self, pattern, entries_subset_key, string):
        ...
        archives_directory_name = self.blog_configuration["path"][
            ...
            dataset.update(
                {
                    ...
                    if type(getattr(entry, attr)) in [str, int, float]
                }
            )
            while "∞":
                try:
        ...
        if not key in self.cache_blog_archives.keys():
            if self.blog_configuration["disable_archives"]:
                ...
                archives = [
                    ...
                    for o in self.entries_per_archives
                    if o.value not in self.disable_threads
                ]
                self.cache_blog_archives[key] = merge(
                    archives, string, separator, pattern
                )
    ...
    def for_blog_metadata(self, pattern, metadata_name, string, separator=""):
        """value"""
        return self.for_blog_metadata_if_exists(
            pattern, metadata_name, string, separator, raise_exception=True
        )

    def for_blog_metadata_if_exists(
        self, pattern, metadata_name, string, separator="", raise_exception=False
    ):
        """value"""
        key = (
            metadata_name + "," + string + "," + separator + "," + str(raise_exception)
        )
        if not key in self.html_for_metadata:
            if not metadata_name in self.blog_configuration.keys():
                if raise_exception:
                    ...
                    raise VenCException(
                        ("blog_has_no_metadata_like", metadata_name), pattern
                    )
            ...
            if type(self.blog_configuration[metadata_name]) != list:
                if raise_exception:
                    ...
                    raise VenCException(
                        ("blog_metadata_is_not_a_list", metadata_name), pattern
                    )

                self.html_for_metadata[key] = ""

            self.html_for_metadata[key] = merge(
                [{"value": v} for v in self.blog_configuration[metadata_name]],
                string,
                separator,
                pattern,
            )
    ...
    def for_entry_metadata(self, pattern, metadata_name, string, separator=""):
        """value"""
        return self.for_entry_metadata_if_exists(
            pattern, metadata_name, string, separator, raise_exception=True
        )

    def for_entry_metadata_if_exists(
        self, pattern, metadata_name, string, separator="", raise_exception=False
    ):
        """value"""
        entry = self.requested_entry
        ...
        if not key in entry.html_for_metadata:
            try:
                ...
                if not type(l) in [list, tuple]:
                    ...
                    raise VenCException(
                        ("entry_metadata_is_not_a_list", metadata_name, entry), pattern
                    )
                ...
                if raise_exception:
                    ...
                    raise VenCException(
                        ("entry_has_no_metadata_like", metadata_name), pattern
                    )
                else:
    ...
    def get_blog_metadata_tree(
        ...
        """value,tree"""
        return self.get_blog_metadata_tree_if_exists(
            pattern,
            metadata_name,
            open_node,
            open_branch,
    ...
    def get_blog_metadata_tree_if_exists(
        self,
        pattern,
        metadata_name,
        open_node,
        ...
        self.html_tree_for_blog_metadata[key] = self.tree_for_metadata(
            self.blog_configuration[metadata_name],
            open_node,
            open_branch,
            value_childs,
            value,
    ...
    def get_entry_metadata_tree(
        ...
        """value,childs"""
        return self.get_entry_metadata_tree_if_exists(
            pattern,
            metadata_name,
            open_node,
            open_branch,
    ...
    def get_entry_metadata_tree_if_exists(
        self,
        pattern,
        metadata_name,
        open_node,
        ...
        return self.tree_for_metadata(
            getattr(entry, metadata_name),
            open_node,
            open_branch,
            value_childs,
            value,
    ...
    def tree_for_metadata(
        ...
        try:
            items = [
                open_branch + value.format(**{"value": item}) + close_branch
                if type(item) != dict
                else open_branch
                + value_childs.format(
                    **{
                        "value": tuple(item.keys())[0],
                        "childs": self.tree_for_metadata(
                            tuple(item.values())[0],
                            open_node,
                            open_branch,
                            value_childs,
    ...
    def get_flattened_categories(self, pattern, string, separator, from_entry=False):
        ...
        key = (
            ...
            + separator
            + "::"
            + str(self.requested_entry.id if from_entry else None)
        )
        cache = (
            self.requested_entry.html_categories_leaves
            if from_entry
            else self.html_categories_leaves
        )
                ...
                [
                    {
                        ...
                        "count": node.count,
                        "weight": round(
                            node.count / self.categories_weight_tracker.value, 2
                        ),
                        "path": node.path,
                    }
