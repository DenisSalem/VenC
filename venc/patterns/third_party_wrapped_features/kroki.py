...
def kroki_from_file(
    pattern, endpoint, image_format, filename, provider="https://kroki.io/"
):
...
def kroki(pattern, endpoint, image_format, code, provider="https://kroki.io/"):
    ...
    filename = (
        "kroki_" + hashlib.md5(code.encode("utf-8")).hexdigest() + "." + image_format
    )
    import os
                ...
                if stream.getcode() != 200:
                    ...
                    raise VenCException(
                        (
                            ...
                            "API Error: {0}".format(stream.getcode()),
                        ),
                        pattern,
                    )
