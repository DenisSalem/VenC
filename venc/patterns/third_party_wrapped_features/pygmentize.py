...
def get_style_sheets(pattern):
    ...
    for filename in code_highlight.includes.keys():
        output += (
            '<link rel="stylesheet" href="\x1a' + filename + '" type="text/css" >\n'
        )
...
def highlight_include(pattern, langage, display_line_numbers, filename):
    from venc.patterns.non_contextual import include_file
...
def highlight(pattern, langage, display_line_numbers, input_code):
    ...
    try:
        ...
        formatter = pygments.formatters.HtmlFormatter(
            style=code_highlight._style,
            linenos=(True if display_line_numbers == "True" else False),
            cssclass=name,
        )

        result = (
            '<div class="__VENC_PYGMENTIZE_WRAPPER__">'
            + pygments.highlight(input_code.replace("\:", ":"), lexer, formatter)
            .replace(".:", "&period;:")
            .replace(":.", ":&period;")
            + "</div>"
        )
