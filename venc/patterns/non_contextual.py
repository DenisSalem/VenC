...
from venc.helpers import SafeFormatDict
...
def set_color(pattern, color, string):
    return (
        '<span class="__VENC_TEXT_COLOR__" style="color: '
        + color
        + ';">'
        + string
        + "</span>"
...
def include_file(pattern, filename, *argv, raise_error=True):
    ...
    if filename == "":
        ...
        raise VenCException(
            ("wrong_pattern_argument", "path", filename, "include_file"),
            pattern,
            pattern.root.string,
        )
    ...
    paths = (
        "includes/" + filename,
        get_base_dir() + "/share/VenC/themes_includes/" + filename,
    )
    for path in paths:
        if shutil.os.path.exists(path):
            try:
                ...
                raise VenCException(
                    ("wrong_permissions", path), pattern, pattern.root.string
                )
    ...
    if include_string == None:
        ...
        raise VenCException(
            (
                ...
                + ":.\n"
                + "\n".join((messages.file_not_found.format(path) for path in paths)),
            ),
            pattern,
            pattern.root.string,
        )
    ...
    if len(argv) > 1:
        return include_string.format_map(
            SafeFormatDict(
                **{
                    "venc_arg_" + str(index): argv[index]
...
def escape_walk(root, node):
    for pattern in node.sub_patterns[::-1]:
        ...
        node.payload[pattern.payload_index] = (
            node.payload[pattern.payload_index][: pattern.o]
            + ".:"
            + ("::".join(pattern.payload))
            + ":."
            + node.payload[pattern.payload_index][pattern.c :]
        )
