...
def dispatcher(dispatcher_id, process, sub_chunk_len, send_in, recv_out):
    output_context = []
    from venc.datastore.entries import multiprocessing_thread_params
    ...
    try:
        while len(chunked_filenames[dispatcher_id]):
            ...
            chunked_filenames[dispatcher_id] = chunked_filenames[dispatcher_id][
                sub_chunk_len:
            ]
...
def worker(worker_id, send_out, recv_in, single_process_argv=None):
    ...
    notify(
        ...
        prepend="│  "
        + ("└─ " if worker_id == worker_params["workers_count"] - 1 else "├─ "),
    )
...
def finish(worker_id):
    ...
    multiprocessing_thread_params["entries"] += multiprocessing_thread_params[
        "chunked_filenames"
    ][worker_id]
