...
def dispatcher(dispatcher_id, process, sub_chunk_len, send_in, recv_out):
    output_context = []
    from venc.parallelism.export_entries import thread_params

    try:
        while len(thread_params["worker_context_chunks"][dispatcher_id]):
            current = thread_params["worker_context_chunks"][dispatcher_id][
                :sub_chunk_len
            ]
            thread_params["worker_context_chunks"][dispatcher_id] = thread_params[
                "worker_context_chunks"
            ][dispatcher_id][sub_chunk_len:]
        ...
        send_in.send([])
        (
            thread_params["code_highlight_includes"][dispatcher_id],
            thread_params["non_parallelizable"][dispatcher_id],
        ) = recv_out.recv()
...
def worker(worker_id, send_out, recv_in, process_argv=None):
    ...
    from venc.markup_languages import process_markup_language
    ...
    notify(
        ("start_thread", worker_id + 1),
        prepend="│  " + ("└─ " if worker_id == datastore.workers_count - 1 else "├─ "),
    )
    ...
    while len(chunk):
        for entry in chunk:
            ...
            if (
                entry.content.has_non_parallelizables
                or entry.preview.has_non_parallelizables
                or entry.html_wrapper.has_non_parallelizables
                or entry.atom_wrapper.has_non_parallelizables
                or entry.rss_wrapper.has_non_parallelizables
            ):
...
def finish(worker_id):
    ...
    for key in thread_params["code_highlight_includes"][worker_id].keys():
        if not key in code_highlight.includes.keys():
            code_highlight.includes[key] = thread_params["code_highlight_includes"][
                worker_id
            ][key]
