    ...
    try:
        ...
        for item in folder:
            if os.path.isdir(origin + "/" + item):
                ...
                shutil.copy(
                    origin + "/" + item, os.getcwd() + "/blog/" + destination + item
                )
    except:
        raise
...
def get_base_dir():
    return os.path.dirname(sys.modules["venc"].__file__)
    ...
    for template_path in templates_paths:
        try:
            ...
            if len(parted) != 2:
                ...
                raise VenCException(
                    ("possible_malformed_entry", template_path, cause),
            ...
            try:
                ...
                raise VenCException(
                    ("possible_malformed_entry", template_path, ""),
                    context=template_path,
                    extra=str(e),
                )
            ...
            from venc.exceptions import MissingTemplateArguments
