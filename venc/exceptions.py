...
class VenCException(Exception):
    ...
    def flatten(self, highlight=None):
        ...
        while len(patterns):
            ...
            patterns = [
                pattern for pattern in patterns if not self.__apply_flatten(pattern)
            ]
        ...
        if type(highlight) == Pattern:
            from venc.patterns.non_contextual import escape

            faulty_pattern = (
                ".:"
                + highlight.payload[0]
                + ("::" if len(highlight.payload[1:]) else "")
                + escape(highlight)
                + ":."
            )

            self.extra = self.extra.replace(
                faulty_pattern, "\033[91m" + faulty_pattern + "\033[0m"
            )

    def __apply_flatten(self, pattern):
        if self.extra.find(pattern.ID) > 0:
            self.extra = self.extra.replace(
                pattern.ID, ".:" + ("::".join(pattern.payload)) + ":."
            )
            return True
...
class PatternsCannotBeUsedHere(VenCException):
    def __init__(self, patterns):
        ...
        for pattern in patterns:
            message = (
                message
                + messages.you_cannot_use_this_pattern_here.format(
                    pattern.payload[0], pattern.root.context
                )
                + "\n"
            )
    ...
    def __init__(self, string_under_processing, op, cp):
        ...
        if too_many_opening_symbols:
            m = (
                ...
                string_under_processing.context,
                len_op - len_cp,
            )
        else:
            m = (
        ...
        s = s.replace(
            "\x00\x00",
            "\033[91m" + (".:" if too_many_opening_symbols else ":.") + "\033[0m",
        )
...
class MissingTemplateArguments(VenCException):
    def __init__(self, template_name, key_error):
        ...
        super().__init__(
            ("this_template_need_the_following_argument", template_name, str(key_error))
        )
...
class VenCSyntaxError(VenCException):
    def __init__(self, string_under_processing, o, c):
        ...
        self.extra = (
            self.extra[:o] + "\033[91m" + self.extra[o:c] + "\033[0m" + self.extra[c:]
        )
