...
from venc.datastore.entry import yield_entries_content
...
def new_entry(params):
    ...
    try:
        entry["ID"] = (
            max([int(filename.split("__")[0]) for filename in yield_entries_content()])
            + 1
        )
    ...
    content = {"authors": [""], "categories": [""], "title": entry_name}
    ...
    output_filename = (
        ...
        + entry_date
        + "__"
        + quirk_encoding(entry["title"]).replace(" ", "_").replace("/", "-")
    )
    ...
    try:
        if not "text_editor" in blog_configuration.keys():
            ...
            command = [
                str(arg) for arg in blog_configuration["text_editor"] if arg != ""
            ]
...
def new_blog(blog_names):
    ...
    for folder_name in blog_names:
        ...
        stream = codecs.open(
            folder_name + "/" + "blog_configuration.yaml", "w", encoding="utf-8"
        )
        default_configuration["blog_name"] = folder_name
        yaml.dump(
            default_configuration, stream, default_flow_style=False, allow_unicode=True
        )
