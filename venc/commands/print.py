...
USAGE = (
    "\033[92m"
    + messages.full_documentation_there.format("https://venc.software")
    + "\033[0m\n"
    + "-v\t--version\n"
    + "-nb\t--new-blog <"
    + messages.arg_blog_name.format("1")
    + "> ["
    + messages.arg_blog_name.format("2")
    + "] [ ... ]\n"
    + "-ne\t--new-entry <"
    + messages.arg_entry_name
    + "> ["
    + messages.arg_template_name
    + "]\n"
    + "-xb\t--export-blog ["
    + messages.theme_name
    + "]\n"
    + "-ex\t--edit-and-xport <"
    + messages.arg_input_filename
    + "> ["
    + messages.theme_name
    + "]\n"
    + "-s\t--serv\n"
...
def template_arguments(params):
    from venc.exceptions import MissingTemplateArguments, VenCException
    from venc.helpers import get_template
            ...
            if len(template_arguments.keys()) == 0:
                ...
                for arg in template_arguments.keys():
                    template = template.replace(
                        arg, "\033[92m{" + template_arguments[arg] + "}\033[0m"
                    )
...
def print_themes(params):
    ...
    for theme in os.listdir(themes_folder):
        if "config.yaml" in os.listdir(themes_folder + theme) and not os.path.isdir(
            themes_folder + theme + "/config.yaml"
        ):
            config = yaml.load(
        ...
        from venc.prompt import msg_format

        print("- " + msg_format["GREEN"] + theme + msg_format["END"] + ":", description)


def version(params):
    from venc import venc_version
