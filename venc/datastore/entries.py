...
class Entries:
        ...
        try:
            if self.workers_count > 1:
                ...
                for i in range(0, self.workers_count):
                    multiprocessing_thread_params["chunked_filenames"].append(
                        filenames[: self.chunks_len]
                    )
                ...
                from venc.parallelism.build_entries import dispatcher, finish, worker
    ...
    def get_entries_for_given_date(self, value, reverse):
        ...
        for entry in (
            self.entries_per_archives[index].related_to[::-1]
            if reverse
            else self.entries_per_archives[index].related_to
        ):
