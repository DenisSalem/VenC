...
from venc.patterns.processor import PatternTree
...
class Entry:
    def __init__(self, filename, paths):
        ...
        if len(entry_parted) == 2:
            entry_parted = [entry_parted[0]] + entry_parted[1].split(
                "---VENC-END-PREVIEW---\n"
            )
            if len(entry_parted) == 3:
                ...
                try:
                    ...
                    raise VenCException(
                        ("possible_malformed_entry", filename, ""),
                        context=filename,
                        extra=str(e),
                    )
                ...
                self.preview = PatternTree(
                    entry_parted[1],
                    filename + ":preview",
                    False if markup_language == "none" else True,
                )
                self.content = PatternTree(
                    entry_parted[2],
                    filename + ":content",
                    False if markup_language == "none" else True,
                )
                ...
                cause = messages.missing_separator_in_entry.format(
                    "---VENC-END-PREVIEW---"
                )
                from venc.exceptions import VenCException

                raise VenCException(
                    ("possible_malformed_entry", filename, cause), context=filename
                )
        else:
            from venc.l10n import messages
        ...
        for key in metadata.keys():
            if not key in ("authors", "categories", "title"):
                if metadata[key] != None:
                    ...
                    notify(
                        ("invalid_or_missing_metadata", key, filename), color="YELLOW"
                    )
                    setattr(self, key, "")
        ...
        if type(metadata["authors"]) != list:
            ...
            raise VenCException(
                ("entry_metadata_is_not_a_list", "authors", self.id), context=filename
            )

        self.authors = tuple(metadata["authors"])

        params = {"entry_id": self.id, "entry_title": self.title}
        ...
        self.path = (
            "\x1a"
            + self.sub_folder
            + quirk_encoding(paths["entry_file_name"].format(**params))
        )
        ...
        if type(metadata["categories"]) != list:
            ...
            raise VenCException(
                ("entry_metadata_is_not_a_list", "categories", self.id),
                context=filename,
            )
...
def get_latest_entryID():
    entries_list = sorted(
        yield_entries_content(), key=lambda entry: int(entry.split("__")[0])
    )
