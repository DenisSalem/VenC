...
def setup_sub_folder(blog_configuration, key):
    ...
    blog_configuration["path"][key] = (
        (path if path[-1] == "/" else path + "/") if (path != "/" and len(path)) else ""
    )
...
def get_blog_configuration():
    ...
    try:
        ...
        for field in mandatory_fields:
            if field not in blog_configuration.keys():
                from venc.prompt import die

                die(("missing_mandatory_field_in_blog_conf", field))

        if (
            "blog_keywords" in blog_configuration.keys()
            and type(blog_configuration["blog_keywords"]) is not list
            and not blog_configuration["blog_keywords"] is None
        ):
            from venc.prompt import die
        ...
        for field in mandatory_fields:
            if field not in blog_configuration["path"].keys():
                from venc.prompt import die
        ...
        if "https://schema.org" not in blog_configuration:
            blog_configuration["https://schema.org"] = {}

        if not blog_configuration["markup_language"] in [
            "none",
            "Markdown",
            "reStructuredText",
            "asciidoc",
            ...
            die(
                (
                    ...
                    blog_configuration["markup_language"],
                    "blog_configuration.yaml",
                )
            )

        if ("sort_by" not in blog_configuration.keys()) or blog_configuration[
            "sort_by"
        ] in ["", None]:
            blog_configuration["sort_by"] = "id"

        if (
            type(blog_configuration["blog_url"]) is str
            and blog_configuration["blog_url"][-1:] == "/"
        ):
            blog_configuration["blog_url"] = blog_configuration["blog_url"][:-1]

        if (
            "disable_threads" in blog_configuration.keys()
            and type(blog_configuration["disable_threads"]) != list
            and blog_configuration["disable_threads"] != None
        ):
            from venc.prompt import die
        ...
        from venc.prompt import die, notify
