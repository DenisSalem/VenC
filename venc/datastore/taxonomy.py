...
from venc.datastore.metadata import MetadataNode, WeightTracker
...
class Taxonomy:
    ...
    def build_tree(
        ...
        category_directory_name = get_blog_configuration()["path"][
        ...
        for item, sub_items in flatten_current_level(input_list):
            ...
            match = None
            try:
                path = sub_folders + quirk_encoding(
                    category_directory_name.format(**{"category": item})
                )
                ...
                die(
                    (
                        "invalid_variable_name_in_setting",
                        str(e),
                        "category_directory_name",
                    )
                )
            ...
            if match == None:
                metadata = MetadataNode(
                    item, entry_id, quirk_encoding(path), weight_tracker
                )
            ...
            if len(sub_items):
                self.build_tree(
                    entry_id,
                    sub_items,
                    match.childs if match != None else metadata.childs,
                    blog_output_leaves,
                    weight_tracker,
                    path,
                )
    ...
    def extract_leaves(self, filter_by_entry_id, branch=None):
        ...
        output = [
            category for category in filter_categories(branch, filter_by_entry_id)
        ]
