...
class ChaptersThread(Thread):
    def __init__(self):
        ...
        self.folder_name = self.datastore.blog_configuration["path"][
    ...
    def setup_chapters_context(self, i, top, len_top):
        ...
        notify(
            ("exception_place_holder", node.index + " " + entry.title + "..."),
        ...
        folder_name = quirk_encoding(
            self.folder_name.format(
                **{"chapter_name": entry.title, "chapter_index": node.index}
            )
        )
    ...
    def do(self, top=None):
        ...
        for chapter_index in range(0, len(top)):
            ...
            self.organize_entries(
                [entry] + self.extract_sub_chapters(chapter.sub_chapters)
            )
            ...
            if len(chapter.sub_chapters) > 0:
                self.indentation_level += (
                    "   " if len(top) - 1 == chapter_index else "│  "
                )
