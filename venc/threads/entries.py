...
class EntriesThread(Thread):
    ...
    def for_pages(self, node, length, string, separator):
        ...
        for i in range(0, length):
            next_entry = (
                None
                if self.current_entry_index >= len(self.entries) - 2
                else self.entries[self.current_entry_index + 1]
            )
            previous_entry = (
                None
    ...
    def setup_context(self, entry):
        ...
        self.relative_origin = str(
            "".join(["../" for p in export_path.split("/")[1:] if p != ""])
        ).replace("//", "/")
    ...
    def write_file(self, output, file_id):
        ...
        stream = codecs.open(written_path, "w", encoding="utf-8")
        stream.write(output)
        stream.close()
        ...
        if not written_path in [t[2] for t in self.known_written_path]:
            self.known_written_path.append(
                (self.current_entry.id, self.current_entry.title, written_path)
            )
            ...
            die(
                (
                    ...
                    "\n".join(
                        [
                            "\t- #" + str(t[0]) + " " + t[1]
                            for t in self.known_written_path
                            if t[2] == written_path
                        ]
                    ),
                )
