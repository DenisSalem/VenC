...
class CategoriesThread(Thread):
    ...
    def do_feeds(self, entries, node, indentation_type):
        entries = sorted(entries, key=lambda entry: entry.id, reverse=True)[
            0 : self.datastore.blog_configuration["feed_length"]
        ]

        if (not self.disable_rss_feed) or (not self.disable_atom_feed):
            from venc.threads.feed import FeedThread

        if not self.disable_rss_feed:
            FeedThread(
                "rss",
                "├" if not self.disable_atom_feed or len(node.childs) else "└",
                self.indentation_level + indentation_type,
            ).do(entries, self.export_path, self.relative_origin)
    ...
    def setup_category_context(self, i, root, len_root):
        ...
        notify(
            ("exception_place_holder", node.value + "..."),
            prepend=self.indentation_level + tree_special_char + "─ ",
        )
        ...
        self.relative_origin = "".join(
            ["../" for f in self.export_path.split("/")[1:] if f != ""]
        ).replace("//", "/")
    ...
    def do(self, root=None):
        ...
        for i in range(0, len_root):
            node, export_path, category_value = self.setup_category_context(
                i, root, len_root
            )
            if node == None:
                continue
            ...
            # do actual context
            entries = [
                entry for entry in self.datastore.entries if entry.id in node.related_to
            ]
            self.organize_entries(
                entries[::-1]
                if self.datastore.blog_configuration["reverse_thread_order"]
                else entries
            )
            super().do()
