...
from venc.patterns.third_party_wrapped_features.pygmentize import get_style_sheets
...
def undefined_variable(match):
    ...
    die(
        ("undefined_variable", match, current_source.ressource),
        extra=current_source.string.replace(
            match[1:-1], "\033[91m" + match[1:-1] + "\033[0m"
        ),
    )
...
class Thread:
        ...
        self.processor.set_patterns(
            {
                ...
                for key, value, in patterns_map.CONTEXTUALS.items()
            }
        )
    ...
    def get_random_number(self, pattern, min_value, max_value, precision):
        ...
        try:
            v = float(min_value) + random.random() * (
            ...
            faulty_arg_name = {v: k for k, v in locals().items()}[
                e.args[0].split("'")[1]
            ]
    ...
    def for_pages(self, pattern, length, string, separator):
        ...
        for page in self.pages:
            if (not page_number < self.current_page - length) and (
                not page_number > self.current_page + length
            ):
                try:
                    output += (
                        string.format(
    ...
    def get_entry_content(self, pattern):
        if not hasattr(self, "current_entry"):
            from venc.exceptions import PatternsCannotBeUsedHere
    ...
    def preview_if_in_thread_else_content(self, pattern):
        ...
        if self.in_thread:
            ...
            self.processor.process(
                preview, Pattern.FLAG_CONTEXTUAL, id(pattern.payload[0])
            )
            return preview.string
            ...
            content = deepcopy(self.current_entry.content)
            self.processor.process(
                content, Pattern.FLAG_CONTEXTUAL, id(pattern.payload[0])
            )
            return content.string
    ...
    def format_filename(self, value):
        try:
            return quirk_encoding(
                self.filename.format(**{"page_number": value if value != 0 else ""})
            )
    ...
    def post_iteration(self):
        ...
        for column in self.columns:
            self.output += (
                self.column_opening.format(self.columns_counter)
                + column
                + self.column_closing
            )
        ...
        self.write_file(
            self.output.replace("\x1a", self.relative_origin), self.page_number
        )
    ...
    def do(self):
        ...
        if self.pages_count == 0:
            ...
            stream = codecs.open(
                self.export_path + "/" + self.format_filename(0), "w", encoding="utf-8"
            )
            stream.write(output.replace("\x1a", self.relative_origin))
            stream.close()
        else:
